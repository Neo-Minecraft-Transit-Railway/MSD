...
public class InitClient {
    ...
    public static void init() {
        ...
        REGISTRY_CLIENT.eventRegistryClient.registerStartClientTick(() -> {
            ...
            if (ClientPlayerEntity.isInstance(clientPlayerEntity) && lastUpdatePacketMillis >= 0 && getGameMillis() - lastUpdatePacketMillis > 500) {
                final MSDDataRequest dataRequest = new MSDDataRequest(clientPlayerEntity.getUuid().toString(), org.mtr.mod.Init.blockPosToPosition(MinecraftClient.getInstance().getGameRendererMapped().getCamera().getBlockPos()), MinecraftClientHelper.getRenderDistance() * 16L);
